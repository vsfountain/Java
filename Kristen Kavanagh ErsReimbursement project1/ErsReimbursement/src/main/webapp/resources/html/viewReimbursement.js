...
function setValues(sw) {
	...
	for(let i = 0; i < sw.length; i++){
		...
		if(sw[i].remb_Status_Id == '3'){
			...
			approveForm.setAttribute('action', 'updateapprovedReimbursement.ERSServlet');
			...
			declineForm.setAttribute('action', 'updatedeclinedReimbursement.ERSServlet');
...
function sortTableAddEvent() {
	...
	for(var i = 0; i<statusFilters.length;i++) {
		console.log('a');
		statusFilters[i].addEventListener("click", /*sortTable()*/function(){
			sortTable();
		});
		console.log('a');
	}
	  ...
	  while (switching) {
	    ...
	    for(let i = 0; i<rows.length; i++) {
	    	...
	    	if(selectedF == "All") {
	    		...
	    		console.log(rows[i].getElementsByTagName("td")[7]);
	    		console.log(rows[i].getElementsByTagName("td")[7].childNodes[0].childNodes[0]);
	    		if(rows[i].getElementsByTagName("td")[7].textContent != "2" && rows[i].getElementsByTagName("td")[7].textContent != "1") {
	    			console.log('3');
	    			rows[i].style.visibility = 'visible';
	    		}else {
	    			rows[i].style.visibility = 'hidden';
