...
function populateReimbTable(reimbs) {
	...
	for (let reimb in reimbs) {
		...
		if (reimbs[reimb].status == 0) {
			...
			cell9.innerHTML = `<form id = "approve" method="POST" name ="approve" action="approve.json">
									<button id ="approveButton" class="btn btn-primary" name="approveButton">Approve</button>
								</form>
								<form id = "deny" method="POST" name ="deny" action="deny.json">
									<button id ="denyButton" class="btn btn-primary" name="denyButton">Deny</button>
								</form>`;
		}
	}
}
